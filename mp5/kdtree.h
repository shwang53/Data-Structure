...
class KDTree
{
  ...
  public:
     ...
     *     Point<3> a(1, 2, 3);
     *     Point<3> b(3, 2, 1);
     *     cout << smallerDimVal(a, b, 0) << rightl; // should print true
     *     cout << smallerDimVal(a, b, 2) << rightl; // should print false
     *     cout << smallerDimVal(a, b, 1) << rightl; // based on operator<, this should be true
     *
     ...
     *     cout << shouldReplace(target, currentBest1, possibleBest1) << rightl; // true
     *     cout << shouldReplace(target, currentBest2, possibleBest2) << rightl; // false
     *     cout << shouldReplace(target, currentBest3, possibleBest3) << rightl;
     ...
     * the containing hyperrectangle. At then right of first step of the
  ...
  private:
    ...
    vector<Point<Dim>> theArray;

    //helper function to destroy whole tree
    void treeDestroy(KDTreeNode * node);

    // helper functions to findNearestNeighbor
    Point<Dim> helper_findNearest(const Point<Dim>& query, KDTreeNode* theNode, int dimension) const;

    //helper function to build KD tree.
    KDTreeNode * buildKDTree(vector<Point<Dim>>& theArray, int left, int right, int dimension);

    //helper function to quick + select (+ partition).
    void quick_select(vector<Point<Dim>>& theArray, int left, int right, int median, int dimension);
    int partition(vector<Point<Dim>>& theArray, int left, int right, int pivot, int dimension);
